							...
							<?php
								$kullanicial	=mysql_query("select * from kullanici where yetki='2' and id='".$kim["id"]."'");
									while($al=mysql_fetch_assoc($kullanicial)){
										$ogid	=$al["id"];
										$adi	=$al["adsoyad"];
										$brans	=mysql_fetch_assoc(mysql_query("select * from branslar where idogretmen='$ogid'"));
					
							?>
						<option value="<?php echo $ogid;?>"><?php echo $adi." - ".$brans["bransi"];?></option>
                        <?php
						}
						?>
                        </select>
...
?>
	<?php
		if($_POST["kayit"]){
			...
			$kaydetyok		=mysql_query("update liste set yoklama='$yokla[$a]' where lid='$kul[$a]' ");
...
<div class='box-header orange-background'>
	...
	$cektir		=mysql_query("SELECT * FROM liste inner join kullanici on liste.ogrenci=kullanici.id where liste.tarih='$rtarih' and liste.saat='$saat'");		
		...
		$ogrenciid	=$q["lid"];
		$rid		=$q["rid"];		
		$konula		=mysql_fetch_assoc(mysql_query("select * from randevular where tarih='$rtarih' and saat='$saat'"));
		$adsoyad		=$q["adsoyad"];
		?><tr>    
