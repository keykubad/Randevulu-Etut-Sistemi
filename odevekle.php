...
if($_POST){
	...
	if(($ogrtmen=="") || ($sinif=="") || ($konu=="") || ($icerik=="") || ($vtarih=="")){
		...
		$odevle		=mysql_query("insert into odevler (sinif,ogretmeni,odevkonu,odevicerik,verilistarih,teslimtarih,sorucozum)
		values ('$sinif','$ogrtmen','$konu','$icerik','$vtarih','$ttarih','$cozumsayi')");
			if($odevle){
				uyar("Ödev başarıyla eklendi!");
				git("yonetici.php?sayfa=odevekle",3);
        ...
        <div class='box-content'>
            <form class="form form-horizontal" method="post" style="margin-bottom: 0;" /><div style="margin:0;padding:0;display:inline">
                    ...
                    <div class='controls'>
						...
						<?php
							$ogretmen		=mysql_query("select * from kullanici where yetki='2'");
								...
								$adsoyad=$a["adsoyad"];
						?>
						<option value="<?php echo $ids;?>"><?php echo $adsoyad;?></option>
						<?php   } ?>
                        </select>
                    </div>
                </div>
			 ...
			 <div class='control-group'>                   
				 ...
				 <div class='controls'>                       
				 <input class='input-block-level' name="cozumsayi" placeholder='Soru çözüm sayısı' type='text' />	
				 </div>				
				 </div>
				        <div class='control-group'>
