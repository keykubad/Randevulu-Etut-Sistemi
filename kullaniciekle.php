    ...
    <div class='row-fluid'>
        <div class='span6 offset3'>
            <div class='text-center'>
                ...
                <small class='muted'>Bu bölümden sisteminize öğretmen veya öğrenci ekleyebilirsiniz.</small>
            </div>
        </div>
    </div>
</div>
...
//resim boş gelirse default
		if($resimf==""){
				$resimf="yuklenenler/kullanici/defaultresim.png";
		}
$kullanicivarmi	=mysql_query("select * from kullanici where kullaniciadi='$kullanici'");
$bakkullanici	=mysql_num_rows($kullanicivarmi);
	if (($yetki=="") || ($adsoyad=="") || ($kullanici=="") || ($sifre=="") || ($kayittarih=="") || ($resimf=="")){
		uyar("HATA!! Lütfen boş alan bırakmayınız!!");
		git("yonetici.php?sayfa=kullaniciekle",5);
	}elseif($bakkullanici>0){
			uyar("HATA!! Kullanıcı sistemde tanımlı lütfen başka kullanıcı adı tanımlayınız.");
			git("yonetici.php?sayfa=kullaniciekle",5);
	}else{
					...
					//resim kontrol
					if($resimboyut>1000000 || strpos($resimtipi,"php") || strpos($resimtipi,"exe") || strpos($resimtipi,"pdf")){
